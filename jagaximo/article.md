...
### master
Sparkのクラスタを管理するノードです。
各マシンのメモリ、CPU数を元に、渡されたタスクに必要なマシンを選別し、タスクに対するクラスタを構築します。
Sparkは各計算（場合によってはSparkアプリケーションなどとも呼ばれる）に対して必要なExecutor数やCPU数などを設定する事ができます。
それらを管理、判断するのがこのノードになります。

### worker(slave)
Masterの下にいる実作業をするノード郡です。起動する際にmasterに対してメモリ数とCPU数を渡します。

#### driver
実作業をするときの親ノードです。各計算するコードを解釈するノードでもあります。
masterノードによって選別されます。
Scala, Java, Python, Rのいずれかで書かれたコードはここで解釈されます。
その途中、RDDを用いた計算が出た場合、RDDの中のデータを複数のpartitionに分割して、各executorに渡します。
その際に、計算を有向無閉路グラフとして解釈し、実行計画を立てたりするのも、このノードです。

#### executor
分割されたタスクを実行するノードです。
計算を実行し、driverへ結果を返します。
